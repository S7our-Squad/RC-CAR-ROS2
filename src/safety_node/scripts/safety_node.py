...
class SafetyNode(Node):
    ...
    def __init__(self):
        ...
        self.scan_subscription = (
            self.create_subscription(  # Subscribe to the scan topic
        ...
        self.teleop_publisher_ = self.create_publisher(
            AckermannDriveStamped, "teleop", 1000
        )
    ...
    def scan_callback(self, scan_msg):
        ...
        for idx, r in enumerate(scan_msg.ranges):
            ...
            if (
                ...
                / max(
                    self.speed
                    * np.cos(scan_msg.angle_min + idx * scan_msg.angle_increment),
                    0.001,
                )
                < threshold
            ):
        ...
        if emergency_breaking:
            ...
            self.get_logger().info(
                "emergency brake engaged at speed {}".format(self.speed)
            )  # Output to Log
